...
from newhackers import config, stories
...
class StoriesTest(unittest.TestCase):
    def setUp(self):
        self.rdb = stories.rdb = redis.Redis(db=13)
    ...
    def test_get_stories_first_page_cached(self):
        ...
        with mock.patch.object(stories, 'update_page') as update_page:
            self.assertEqual(STORIES_JSON, stories.get_stories('front_page'))
            update_page.assert_not_called()

    def test_get_stories_not_cached(self):
        with mock.patch.object(stories, 'update_page', return_value='stories'
                               ) as update_page:
            self.assertEqual('stories', stories.get_stories('front_page'))
    ...
    def test_get_stories_other_page_cached(self):
        ...
        with mock.patch.object(stories, 'update_page') as update_page:
            update_page.assert_not_called()
            self.assertEqual(STORIES_JSON, stories.get_stories(PAGE_ID))
